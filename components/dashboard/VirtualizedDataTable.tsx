...
interface RowData<T> {
  ...
  selectable: boolean
  striped: boolean
  onSelectRow: (index: number, row: T) => void
  onRowClick?: (row: T) => void
}
...
function Row<T extends Record<string, any>>({
  ...
  return (
    <div style={style}>
      <div
        className={cn(
          "flex items-center border-b border-white/[0.08] transition-colors min-h-[48px] px-4",
      ...
      >
        {selectable && (
          <div className="w-12 mr-4">
            <Checkbox
              checked={isSelected}
              onCheckedChange={() => onSelectRow(index, row)}
...
export function VirtualizedDataTable<T extends Record<string, any>>({
  ...
  // Use virtualized table only for large datasets
  const shouldVirtualize = paginatedData.length > virtualizedThreshold
  ...
  if (!shouldVirtualize) {
    // Render regular table for smaller datasets
    return (
  ...
  return (
    <div className={cn("space-y-4", className)}>
      <div className="rounded-lg border border-white/[0.08] bg-gradient-to-b from-white/[0.02] to-transparent overflow-hidden">
        {/* Header */}
        <div className={cn(
          "flex items-center border-b border-white/[0.08] bg-[rgb(23,24,26)] min-h-[48px] px-4",
          ...
          {columns.map((column) => (
            <div
              key={column.key as string}
              className={cn(
                "flex-1 text-gray-400 cursor-pointer select-none hover:text-white transition-colors px-2",
                column.align === 'center' && 'text-center',
                column.align === 'right' && 'text-right',
                !column.sortable && 'cursor-default hover:text-gray-400'
              )}
              onClick={() => column.sortable && handleSort(column.key as string)}
            >
              <div className={cn(
